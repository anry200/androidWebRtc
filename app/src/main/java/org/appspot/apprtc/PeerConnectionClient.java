...
public class PeerConnectionClient {
  ...
  public interface PeerConnectionEvents {
    ...
    void onIncomingMessage(final String message);
  }
...
//              Log.d(TAG, "Got msg: " + strData + " over " + dataChannel + " *incoming");
//          }
//      });
  }
  ...
  private class PCObserver implements PeerConnection.Observer {
    ...
    public void onDataChannel(final DataChannel dc) {
      ...
      if (dc.label().contains(".txt")) {
          dc.registerObserver(new FileDataChannel(dc));
      } else {
          dc.registerObserver(new TextTransferChannel(dc, events));
      }
    }
  ...
  private static class FileDataChannel implements DataChannel.Observer {
      private DataChannel dc;

      FileChannel channel;

      public FileDataChannel(DataChannel dc) {
          this.dc = dc;
      }
      ...
      public void onStateChange() {
          ...
          if (dc.state().equals(DataChannel.State.OPEN)) {
              //Create new file
              try {
                  //TODO: add passing real file name.
                  String fileName = UUID.randomUUID().toString(); //create file with random name
                  File file = new File(Environment.getExternalStorageDirectory(), fileName);

                  channel = new FileOutputStream(file, false).getChannel();
              } catch (Throwable th) {
                  Log.e(TAG, "Failed open file", th);
              }
          } else if (dc.state().equals(DataChannel.State.CLOSED)) {
              try {
                  channel.close();
              } catch (Throwable th) {
                  Log.e(TAG, "Failed close file", th);
              }
          }
      }
      ...
      public void onMessage(DataChannel.Buffer buffer) {
          if (buffer.binary) {
              Log.d(TAG, "Received binary msg over " + dc.label());
              try {
                  channel.write(buffer.data);
              } catch (Throwable th) {
                  //ignore for now
                  Log.e(TAG, "Failed add data to file", th);
              }
          }
      }
  }

  private static class TextTransferChannel implements DataChannel.Observer {
      private DataChannel dc;
      private PeerConnectionEvents events;

      public TextTransferChannel(DataChannel dc, PeerConnectionEvents events) {
          this.dc = dc;
          this.events = events;
      }
      ...
      public void onMessage(DataChannel.Buffer buffer) {
          ...
          Log.d(TAG, "Got msg: " + strData + " over " + dc.label());
          events.onIncomingMessage(strData);
      }
  }
  ...
  public void sendMessage(final String message) {
        executor.execute(new Runnable() {
          ...
          public void run() {
              getDataChannel()
                      .send(new DataChannel.Buffer(str_to_bb(message, Charset.defaultCharset()), false));
          }
      });
  }

  private DataChannel.Buffer fromText(String text) {
      return new DataChannel.Buffer(str_to_bb(text, Charset.defaultCharset()), false);
  }
  ...
  public void sendFile(final String path) {
      executor.execute(new Runnable() {
          ...
          public void run() {
              ...
              init.id = peerConnectionParameters.dataChannelParameters.id;
              init.protocol = peerConnectionParameters.dataChannelParameters.protocol;
              dataChannel = peerConnection.createDataChannel("AppRtcDemo dataChannel for file " + path, init);

              //send text message
              //dataChannel.send(fromText("Save file start"));
              ...
              try {
                  ...
                  //if file size is smaller than buffer.
                  int bufSize = 1024;
                  if (fileSize < bufSize) {
                  ...
                  Log.d(TAG, "send file end");
              } catch (Throwable th) {
                  Log.e(TAG, "Failed sent file", th);
                  //ignore for now
              }

                //send text
              dataChannel.send(fromText("Save file end"));
          }
      });
  }
